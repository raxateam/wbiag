 ...
 *
*/
public class ClockAlertLinkSource extends AbstractRowSource {
    private static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(ClockAlertLinkSource.class);
    ...
    public static final String COL_TIMESHEET = "TIMESHEET";

	{
		...
		rs.add(COL_TIMESHEET,CharType.get(200));
    ...
    */
    public ClockAlertLinkSource(DBConnection dbConnection, HashMap alertParams) throws AccessException  {
...
// TODO: implement: pay group, calc group
		...
		try {
			...
			while(resultSet.next()) {
				...
				if(clockingIssueExists) {
				   ...
				   row.setValue(COL_TIMESHEET, getTimesheetLink(employeeId, currentDate, dbConnection));
	...
	/** @return a link to the timesheet of the specified employee with one day on each side of the specified day. */
	protected String getTimesheetLink(int employeeId, Date wrksDate, DBConnection dbConnection) {
		try {
			Date startDate = DateHelper.addDays(wrksDate, -1);
			Date endDate = DateHelper.addDays(wrksDate, 1);
			String startDateString = DatetimeType.FORMAT.format(startDate);
			String endDateString = DatetimeType.FORMAT.format(endDate);
			String tsLinkParams = "&DATE_SELECT=7&AUTH_SELECT=0&VIEW_SELECT=0&SUBMIT_PARAMS=T&ORDER_SELECT=0";

			StringBuffer tsLink = new StringBuffer();
			tsLink.append("<a href=# onclick=\"");
			tsLink.append(Timesheet.getTimesheetLink(dbConnection, employeeId, false, startDateString, endDateString, tsLinkParams,""));
			tsLink.append("\">");
			tsLink.append("Timesheet");
			tsLink.append("</a>");
			if(logger.isDebugEnabled()) { logger.debug("tsLink=[" + tsLink + "]"); }
			return tsLink.toString();
		} catch(Exception ex) {
			logger.error(ex);
