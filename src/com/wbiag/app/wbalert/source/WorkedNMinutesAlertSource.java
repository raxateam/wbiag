...
import com.workbrain.app.ta.db.HourTypeAccess;
import com.workbrain.util.StringHelper;


public class WorkedNMinutesAlertSource extends AbstractRowSource {
	
	///////////////////CONFIGURATION///////////////////
	// For daily checking, check yesterday, or today?
	public static final String DAILY_CHECK = "yesterday";
	///////////////////END OF CONFIG///////////////////
	...
	public static final String PARAM_TIME_PERIOD = "tPeriod";
	
	public static final String PARAM_MIN_EMP_AGE = "minEmpAge";
	...
	private java.util.List tempRows = new ArrayList();
	...
	private boolean minAgeSet = false;
	...
	public WorkedNMinutesAlertSource(DBConnection c, HashMap params)
	...
	private void loadParameters(HashMap params) {
		...
		tPeriod = (String) params.get(PARAM_TIME_PERIOD);
		...
		try {
			if (new Integer((String) params.get(PARAM_MIN_EMP_AGE)) != null)
				minAgeSet = true;
		}
		...
		if (minAgeSet)
			minAge = new Integer((String) params.get(PARAM_MIN_EMP_AGE)).intValue();
		else
			minAge = -1;
	...
	private void processWorkDetail(WorkDetailData wdd, String fullName)
		...
		try {
			wdd.setCodeMapper(CodeMapper.createCodeMapper(conn));
		}
		catch (SQLException e) {
			e.printStackTrace();
		}
		
		// Check validity of time codes
		if (!StringHelper.isEmpty(wrkCode)
				&& ((wrkCodeInclusive== true && !RuleHelper.isCodeInList(wrkCode.toUpperCase(), wdd.getWrkdTcodeName().toUpperCase()))  
					 || (wrkCodeInclusive== false && RuleHelper.isCodeInList(wrkCode.toUpperCase(), wdd.getWrkdTcodeName().toUpperCase())) 	))
			return;
		
		HourTypeAccess hTypeAccess = new HourTypeAccess(conn);
		String hTypeName = ( (HourTypeData) hTypeAccess.load(wdd.getHtypeId()) ).getHtypeName();
		
		// Check validity of hour types
		if (!StringHelper.isEmpty(hType) 
				&& ((hTypeInclusive == true && !RuleHelper.isCodeInList(hType.toUpperCase(), hTypeName.toUpperCase() )) || 
					(hTypeInclusive == false && RuleHelper.isCodeInList(hType.toUpperCase(), hTypeName.toUpperCase() ))))
			return;

		// Return before adding the row if employee has not worked n minutes yet.
		accumulatedTime += wdd.getWrkdMinutes();
		...
		r.setValue("Time Worked (mins)", new Integer(wdd.getWrkdMinutes()).toString());
		tempRows.add(r);
	}
	...
	private String getTime(java.util.Date time) {
