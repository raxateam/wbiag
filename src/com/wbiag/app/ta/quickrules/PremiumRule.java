 ...
 */
public class PremiumRule extends Rule {

    private static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(PremiumRule.class);
    ...
    public void execute(WBData wbData, Parameters parameters) throws Exception {
        ...
        // If the employeeVal parameter is set, then use it.
        	...
        	premiumMinutes = (int) (empValValue * pars.multiple);
        ...
        // Calculate the number of minutes earned for the premium.
            ...
            ***/
            ...
            //Or Started after todays end date
            for (int k = 0, l = wbData.getWorkDetailsForDate(yesterdaysDate).size(); k < l; k++) {
                ...
                } else if (currWdData.getWrkdStartTime().getTime() > todaysDate.getTime()){
                	yesterdaysOvernightWds.add(currWdData);
                }
            }
            ...
            if (doDetails) {
                for (int k = 0; k < yesterdaysOvernightWds.size(); k++) {
                    ...
                    if (isWorkDetailEligible(wd, pars)) {
                        premiumMinutes += getMinutesInRange(wbData, wd, pars);
                    }
                }
            }
     ...
     * @throws Exception
     */
    protected void insertPremium(WBData wbData, int premiumMinutes, ParametersResolved pars) throws Exception {
        ...
        if (premiumMinutes > pars.dayMinMins) {
        	...
        	// Check that we do not exceed daily, weekly maxiums.
            premiumMinutes = applyMaxMinutes(wbData , premiumMinutes, pars);

            // Check that we are still paying the premium, after min/max validations.
            if (premiumMinutes != 0 && premiumMinutes > pars.dayMinMins) {

            	if (pars.useRate) {
            		wbData.insertWorkPremiumRecord(premiumMinutes,
    ...
    /**
     * Checks the premMinutes against the daily and weekly maximums.  Returns the
     * capped duration to use for the premium.
     *
     * @param wbData
     * @param premMins
     * @param dayMaxMins
     ...
     * @throws Exception
     */
    protected int applyMaxMinutes(WBData wbData, int premMins,ParametersResolved pars) throws Exception {

    	int premiumMinutesCapped = premMins;

    	// If a daily max premium was specified, then enforce it.
        if (pars.dayMaxMins != Integer.MIN_VALUE) {

        	if (logger.isDebugEnabled()) {
            	logger.debug("Premium minutes : " + premMins + " - Day max minutes:" + pars.dayMaxMins);
            }

        	// Do not allow greater than dayMaxMins.
            premiumMinutesCapped = Math.min(premMins , pars.dayMaxMins);
        }
     ...
     * @throws Exception
     */
    protected Date getWeekStart(WBData wbData) throws Exception {

    	String firstDayOfWeek = (String) Registry.getVar("system/WORKBRAIN_PARAMETERS/DAY_WEEK_STARTS" );
     ...
     * @throws SQLException
     */
    protected int getMinutesInRange(WBData wbData, WorkDetailData wd,
        ...
        if (pars.startZoneTime == pars.endZoneTime) {
            ...
            if (pars.useHtypeMultiplier) {
                minutes *= codeMapper.getHourTypeById(wd.getHtypeId()).getHtypeMultiple();
            }
