 ...
 *
 */
public class HRRefreshTransactionExtraEmpDefLabTest extends WBInterfaceCustomTestCase {
    private static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(HRRefreshTransactionExtraEmpDefLabTest.class);
    ...
    private final String EMP_DEF_LAB_EXTRA_COL   = "20~NULL DOCKETA~UNPAID~OPERATOR~UAT~0~0|20~NULL DOCKETA~UNPAID~SUPERVISOR~UAT~0~0";
    private final String EMP_DEF_LAB_EXTRA_COL_UPDATE = "15~NULL DOCKETA~UNPAID~OPERATOR~UAT~0~0|15~NULL DOCKETA~UNPAID~SUPERVISOR~UAT~0~0";
    private final String EMP_DEF_LAB_UDFFLAG_COL   = "EDLA_FLAG1=Y~EDLA_UDF1=UDF1|EDLA_FLAG1=Y~EDLA_UDF1=UDF1";
    private final String EMP_DEF_LAB_UDFFLAG_COL_UPDATE = "EDLA_FLAG1=N~EDLA_UDF1=UDF11|EDLA_FLAG1=N~EDLA_UDF1=UDF11";

    private final String typName = "HR REFRESH";
    private final int typId = 1;


    public HRRefreshTransactionExtraEmpDefLabTest(String testName) throws Exception {
        super(testName);
    }

    public static TestSuite suite() {
        TestSuite result = new TestSuite();
        result.addTestSuite(HRRefreshTransactionExtraEmpDefLabTest.class);
        return result;
    }
     ...
     * @throws Exception
     */
    public void testInsertEmpDefLab() throws Exception{

        updateWbintTypeClass(typId , "com.wbiag.app.wbinterface.hr2.HRRefreshTransactionExtraEmpDefLab");
        ...
        List edlab = new EmployeeDefaultLaborAccess(c, cm).load(ed , DateHelper.getCurrentDate());
        assertEquals(6, edlab.size());
        EmployeeDefaultLaborData edlabLast = (EmployeeDefaultLaborData) edlab.get(5);
        assertEquals(20 , edlabLast.getEdlaPercentage());
        assertEquals(cm.getJobByName("SUPERVISOR").getJobId() , edlabLast.getJobId());
        EmployeeDefaultLaborData edlabFirst = (EmployeeDefaultLaborData) edlab.get(0);
        assertEquals("Y", edlabFirst.getEdlaFlag1() );
        assertEquals("UDF1", edlabFirst.getEdlaUdf1() );
     ...
     * @throws Exception
     */
    public void testUpdateEmpDefLab() throws Exception{
        ...
        assertEquals(6, edlab.size());
        EmployeeDefaultLaborData edlabLast = (EmployeeDefaultLaborData) edlab.get(5);
        assertEquals(15 , edlabLast.getEdlaPercentage());
        assertEquals(cm.getJobByName("SUPERVISOR").getJobId() , edlabLast.getJobId());
        EmployeeDefaultLaborData edlabFirst = (EmployeeDefaultLaborData) edlab.get(0);
        assertEquals("N", edlabFirst.getEdlaFlag1() );
    }
    ...
    private String createData(boolean addSODOData){
        ...
        stm += ",";
        stm += ",";
        stm += "," + EMP_DEF_LAB_EXTRA_COL;
        stm += "," + EMP_DEF_LAB_UDFFLAG_COL ;
        return stm;
    }
    ...
    private String createUpdateData(String empName){
        ...
        stm += ",";
        stm += ",";
        stm += "," + EMP_DEF_LAB_EXTRA_COL_UPDATE;
        stm += "," + EMP_DEF_LAB_UDFFLAG_COL_UPDATE ;
        return stm;
    }
