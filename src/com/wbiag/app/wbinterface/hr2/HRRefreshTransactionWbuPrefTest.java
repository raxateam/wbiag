 ...
 *
 */
public class HRRefreshTransactionWbuPrefTest extends WBInterfaceCustomTestCase {
    private static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(HRRefreshTransactionWbuPrefTest.class);
    ...
    private final String WBU_PREF_COL         = "TOC_VIEW_STATE=false~ETM_STARTING_PAGE=/etm/etmMenu.jsp";
    private final String WBU_PREF_COL_UPDATE = "TOC_VIEW_STATE=true~ETM_STARTING_PAGE=/etm/time/timesheet/etmClocks.jsp";

    private final String typName = "HR REFRESH";
    private final int typId = 1;


    public HRRefreshTransactionWbuPrefTest(String testName) throws Exception {
        super(testName);
    }

    public static TestSuite suite() {
        TestSuite result = new TestSuite();
        result.addTestSuite(HRRefreshTransactionWbuPrefTest.class);
        return result;
    }
     ...
     * @throws Exception
     */
    public void testInsertWbuPref() throws Exception{

        updateWbintTypeClass(typId , "com.wbiag.app.wbinterface.hr2.HRRefreshTransactionWbuPref");
        ...
        List wbupList = new RecordAccess(c).loadRecordData(
            new WorkbrainUserPreferenceData(), HRRefreshTransactionWbuPref.TABLE_WORKBRAIN_USER_PREFERENCE,
            "wbu_id = (SELECT wbu_id FROM workbrain_user where emp_id = " + ed.getEmpId() + ")");
        assertNotNull(ed);

        assertTrue(wbupList.size() == 2);
    }
    ...
    public void xUpdate() throws Exception{
        ...
        assertEquals("Timesheet flag", Integer.parseInt(WBU_PREF_COL_UPDATE) , getTSUserTimeSheetFlag(ed.getEmpId()));
    }
